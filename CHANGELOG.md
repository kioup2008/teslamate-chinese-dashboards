...
### 🛠 元事情：升级 memory `feedback_audit_simulate_user.md`

把"用户视角 3 路审计"从一条建议变成强制规则：任何 user-facing 改动 push 前必须显式跑「群晖小白 / 云服务器程序员 / macOS 笔记本小白」3 路审计，没有 = 不发版。v1.6.3 发版即翻车（issue #13 Docker volume 坑 + 后续 30+ finding）就是漏了这条。
